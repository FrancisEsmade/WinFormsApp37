...
using System.Runtime.InteropServices;
    ...
    public partial class Form1 : Form
    {
        ...
        //add button this button will update if ang usa ka user kay mag usob if nasayup and mausob diritcho ang time kung kanus a pud na update
        private void ADD_Click(object sender, EventArgs e)
        {
            try
            {
                if (ListBox.SelectedIndex == -1)//check if the item is selected if walay g iselec ,it show a messsagebox 
                {
                ...
                int index = ListBox.SelectedIndex;
                files = Loading() ?? new List<User>();// it retrieves ang bag ong data 

                if (index < files.Count)//it enures nga sa setedindex kay na exist ba within the loaded list 
                {
